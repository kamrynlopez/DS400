...
Less than 20 are between ages 50-52.

```{r}
runners_speed <- cherry_blossom_sample %>%
  filter(gun > 80)
```

Many runners speed are above 80.
...
no real correlation between age and the amount of time it took to complete.

Positive would represent the older you are the longer it took to complete.

Negative would represent the younger you are the longer it took to complete.
